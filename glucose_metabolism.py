...
def glucose_metabolism_app():
    st.header("Simulations of glucose metabolism")
    optionsSim = ["IVGTT simulation", "OGTT simulation", "Glucose clamp simulation"]
    simulationPage = st.selectbox("Select specific simulator", options=optionsSim)
    ...
    if simulationPage == "IVGTT simulation":
        ...
        Ib = basal_cols[1].number_input(
            "Basal insulin concentration [mU/L]", min_value=1, max_value=40, value=6
        )
        ...
        with disp_par.beta_expander("Display parameter values"):
            ...
            st.latex(
                r"""
                        \begin{array}{lll}
                        T_G &= """
                + str(round(model.parameters[0], 2))
                + r"""& \textrm{[min]} \\
                        k_x &= """
                + format(model.parameters[1], ".2e")
                + r"""& \textrm{[L/(mU.min)]} \\
                        V_G &= """
                + str(round(model.parameters[2], 2))
                + r"""& \textrm{[L/kg]} \\
                        T_X &= """
                + str(round(model.parameters[3], 2))
                + r"""& \textrm{[min]} \\
                        T_I &= """
                + str(round(model.parameters[4], 2))
                + r"""& \textrm{[min]} \\
                        K_{G1} &= """
                + str(round(model.parameters[5], 2))
                + r"""& \textrm{[mU/min per mmol]} \\
                        K_{G2} &= """
                + str(round(model.parameters[6], 2))
                + r"""& \textrm{[mU per mmol]} \\
                        T_P &= """
                + str(round(model.parameters[7], 2))
                + r"""& \textrm{[min]} \\
                        V_I &= """
                + str(round(model.parameters[8], 2))
                + r"""& \textrm{[L/kg]} \\
                        \end{array}
                        """
            )
        ...
        glucose_bolus_min = input_cols[0].slider(
            "Duration of glucose administration [min]",
            min_value=0.5,
            max_value=5.0,
            value=2.0,
        )

        insulin_dose = input_cols[1].slider(
            "Insulin bolus dose [mU/kg]", min_value=1, max_value=50, value=20
        )
        insulin_bolus_min = input_cols[1].slider(
            "Duration of insulin administration [min]",
            min_value=0.5,
            max_value=10.0,
            value=5.0,
        )
        insulin_dosage_time = input_cols[1].slider(
            "Time of insulin administration (from the start) [min]",
            min_value=1,
            max_value=40,
            value=20,
        )
        ...
        def plot_simulation_results():
            ...
            fig.layout.update(
                title_text="IVGTT simulation", xaxis_rangeslider_visible=False
            )
        ...
        with st.beta_expander("Model equations"):
            ...
            st.markdown(
                r"""where  $S^{+}(t)$ has PD controller-like structure in 
        ...
        with disp_par.beta_expander("Display parameter values"):
            ...
            st.latex(
                ...
                + r"""& \textrm{[mU/min per mmol]} \\
                        K_{G2} &= """
                + str(round(model.parameters[9], 2))
                + r"""& \textrm{[mU per mmol]} \\
                        T_P &= """
                + format(model.parameters[10], ".2e")
                + r"""& \textrm{[min]} \\
                        V_I &= """
                + str(round(model.parameters[11], 2))
                + r"""& \textrm{[L/kg]} \\
                        K_{G1m} &= """
                + str(round(model.parameters[12], 2))
                + r"""& \textrm{[-]} \\
                        K_{G2m} &= """
                + str(round(model.parameters[13], 2))
                + r"""& \textrm{[-]} \\
                        \end{array}
                        """
            )
        ...
        BW = st.slider(
            "Bodyweight of a subject [kg]", min_value=50, max_value=120, value=70
        )
        ...
        t, G, I, RaG_iv = model.hyperinsulinemic_euglycemic_glucose_clamp(
            BW, plot=False
        )
        ...
        def plot_simulation_results():
            ...
            fig.append_trace(
                go.Scatter(x=t, y=G, name="Glucose conc. [mmol/L]"), row=1, col=1
            )
            fig.append_trace(
                go.Scatter(x=t, y=I, name="Insulin conc. [mU/L]"), row=2, col=1
            )
            fig.append_trace(
                go.Scatter(x=t, y=RaG_iv, name="Glucose infusion rate [mmol/min/kg]"),
                row=3,
                col=1,
            )
            fig.layout.update(
                title_text="IVGTT simulation",
            ...
            fig.update_yaxes(
                title_text="Glucose infusion rate [mmol/min/kg]", row=3, col=1
            )
            st.plotly_chart(fig)
