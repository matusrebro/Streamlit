...
def fcn_Bergman_extended(
    x, t, p, RaG_iv, d, RaI, Gb, Ib, incretin_effect_model="model1"
):
    ...
    if incretin_effect_model != "none":
        ...
        D_dot = 0
        Ra_dot = 0
    G_dot, X_dot, I_dot = fcn_Bergman(
        [G, X, I], 0, [T_G, Kx, V_G, T_X, T_I, V_I], (RaG_iv + Ra), RaI, S, Gb, Ib
    )
...
class minimal_model:
    ...
    class iv:
        ...
        def simulation(self, t, RaG_iv, RaI, plot=True):
            ...
            for i in range(1, idx_final):
                y = odeint(
                    fcn_Bergman_extended,
                    x[i - 1, :],
                    np.linspace((i - 1) * Ts, i * Ts),
                    args=(
                        self.parameters,
                        RaG_iv[i - 1],
                        0,
        ...
        def ivgtt(
            ...
            RaG_iv[0 : int(glucose_bolus_min / 1)] = (
                glucose_bolus / 1 / glucose_bolus_min
            )
            RaI = np.zeros_like(t, dtype=float)
            RaI[
                int(insulin_dosage_time / 1) : int(insulin_dosage_time / 1)
                + int(insulin_bolus_min / 1)
            ] = (insulin_dose / 1 / insulin_bolus_min)

            x = self.simulation(t, RaG_iv, RaI, plot)

            return t, x[:, 0], x[:, 4]  # t, G, I

        def hyperinsulinemic_euglycemic_glucose_clamp(
            self, BW, insulin_rate=120, plot=True
        ):
            """
    ...
    class oral:
        ...
        def init_model(
            self, Gb, Ib, parameters="normal", incretin_effect_model="model1"
        ):
                ...
                if parameters == "normal":
                    if incretin_effect_model == "model1":
                        self.parameters = np.loadtxt(
                            pathToParameters + "par_normal_oral_inc1.csv"
                        )
                    elif incretin_effect_model == "model2":
                        self.parameters = np.loadtxt(
                            pathToParameters + "par_normal_oral_inc2.csv"
                        )
                    elif incretin_effect_model == "model3":
                        self.parameters = np.loadtxt(
                            pathToParameters + "par_normal_oral_inc3.csv"
                        )
                    elif incretin_effect_model == "model4":
                        self.parameters = np.loadtxt(
                            pathToParameters + "par_normal_oral_inc4.csv"
                        )
                    else:
                        raise ValueError("Invalid value for incretin_effect_model")
                    ...
                    if incretin_effect_model == "model1":
                        self.parameters = np.loadtxt(
                            pathToParameters + "par_t2dm_oral_inc1.csv"
                        )
                    elif incretin_effect_model == "model2":
                        self.parameters = np.loadtxt(
                            pathToParameters + "par_t2dm_oral_inc2.csv"
                        )
                    elif incretin_effect_model == "model3":
                        self.parameters = np.loadtxt(
                            pathToParameters + "par_t2dm_oral_inc3.csv"
                        )
                    elif incretin_effect_model == "model4":
                        self.parameters = np.loadtxt(
                            pathToParameters + "par_t2dm_oral_inc4.csv"
                        )
                    else:
                        raise ValueError("Invalid value for incretin_effect_model")
...
def fcn_Sorensen(x, t, p, Basal, r_IVG, r_IVI):
    ...
    dot_I_H = (
        1
        / VI_H
        * (QI_B * I_B + QI_L * I_L + QI_K * I_K + QI_P * I_PV - QI_H * I_H + r_IVI)
    )
    ...
    MG_PGaR = (
        1
        + 2.10 * np.tanh(4.18 * (1 - 0.62))
        - 2.10 * np.tanh(4.18 * (G_H / GB_H - 0.62))
    )
    MI_PGaR = (
        1
        + 0.61 * np.tanh(1.06 * (1 - 0.47))
        - 0.61 * np.tanh(1.06 * (I_H / IB_H - 0.47))
    )
...
class Sorensen_model:
    ...
    def init_model(
        ...
        Gb,
        Ib,
        parameters=np.loadtxt(pathToParameters + "par_Sorensen_normal.csv"),
        blood_measure="peripheral",
    ):
