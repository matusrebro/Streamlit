...
# prediction N-steps ahead using ARIMA (no input) and recursive least squares algorithm for parameter identification
def arima(y, na, nc, N, fz):
    # data to predict
    # na - order of polynomial of autoregressive part of model
    # nc - order of polynomial of moving average part of model
    # N - prediction horizon
    ...
    for i in range(0, idx_final):
        ...
        # this should remain the same as in arma
        h = np.vstack((-yp, ep))
        y_hat = np.dot(h[:, 0], theta[i - 1, :])
        yhat[i, 0] = y_hat
        e = y1[i, 0] - y_hat
        ...
        yp = np.roll(yp, 1)  # this is still delta
        yp[0, 0] = y1[i, 0]
        ep[0, 0] = e
